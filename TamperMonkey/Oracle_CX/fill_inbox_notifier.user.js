...
(function() {
    ...
    let infoPanel = document.getElementById('fillInboxInfoPanel');
    if (!infoPanel) {
        infoPanel = document.createElement('div');
        infoPanel.id = 'fillInboxInfoPanel';
    ...
    // If we're initializing again, let's clear the old interval just in case though Tampermonkey scope isolation
    // usually keeps variables separate, global window bindings might be better. 
    // We'll rely on the singleton pattern to ensure we don't have multiple loops running
    if (window._autoFillInboxNotifierLoop) {
        clearInterval(window._autoFillInboxNotifierLoop);
    }
    ...
    setTimeout(() => {
        statusEl.innerText = 'Listening...';
        window._autoFillInboxNotifierLoop = setInterval(triggerFillInbox, FILL_INBOX_POLL_INTERVAL);
